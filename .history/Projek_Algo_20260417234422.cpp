...
void cari_buku()
{
    ...
    else
    {
        ...
        for (int f = 0; f < n; f++)
        {
            string temp_cek = buku[f].judul_buku;
            if (temp_benar == temp_cek)
            {
                cout << left << setfill(' ') << setw(3) << i + 1 << " "
                     << left << setfill(' ') << setw(10) << buku[f].id_buku << " "
                     << left << setfill(' ') << setw(15) << buku[f].judul_buku << " "
                     << left << setfill(' ') << setw(6) << buku[f].stok_buku << " "
                     << left << setfill(' ') << setw(17) << buku[f].total_peminjaman << " "
                     << left << setfill(' ') << setw(8) << buku[f].tersedia << " "
                     << left << setfill(' ') << setw(3) << buku[f].terpinjam << " "
                     << endl;
                i++;
            }
        }
