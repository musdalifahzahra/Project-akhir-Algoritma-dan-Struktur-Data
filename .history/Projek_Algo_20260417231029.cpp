...
void menambahkan_data_buku(dataBuku *ptr, int &jumlah)
{
    do
    {
        cout << "Nama Lukisan: ";
        cin.ignore();
        cin.getline((ptr_d_lukisan + n)->lukisan, 25);
    } while ((ptr_d_lukisan + n)->lukisan == "\0");

    do
    {
        cout << "Nama Pelukis: ";
        cin.getline((ptr_d_lukisan + n)->pelukis, 25);
    } while ((ptr_d_lukisan + n)->pelukis == "\0");

    do
    {
        cout << "Harga       : ";
        cin >> (ptr_d_lukisan + n)->harga;
    } while ((ptr_d_lukisan + n)->harga < 1);
...
void melihat_daftar_buku(dataBuku *ptr, int &jumlah)
{
    ...
    for (int i = 0; i < jumlahData; i++)
    {
        cout << left << setfill(' ') << setw(3) << i + 1 << " "
             << left << setfill(' ') << setw(3) << buku[i].id_buku << " "
             << left << setfill(' ') << setw(3) << buku[i].judul_buku << " "
             << left << setfill(' ') << setw(3) << buku[i].stok_buku << " "
             << endl;
